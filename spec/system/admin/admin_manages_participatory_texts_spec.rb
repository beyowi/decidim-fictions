...
describe "Admin manages participatory texts", type: :system do
  ...
  def import_document
    ...
    expect(page).to have_content "The following sections have been converted to fictions. Now you can review and adjust them before publishing."
    expect(page).to have_content "Preview participatory text"
  end
  ...
  def publish_participatory_text
    ...
    expect(page).to have_content "All fictions have been published"
  end

  def validate_published
    fictions = Decidim::Fictions::Fiction.where(component: current_component)
    titles = [
    ...
    expect(fictions.count).to eq(titles.size)
    expect(fictions.published.count).to eq(titles.size)
    expect(fictions.published.order(:position).pluck(:title)).to eq(titles)
  end

  def edit_participatory_text_body(index, new_body)
    fill_in(
      "preview_participatory_text_fictions_attributes_#{index}_body",
      with: new_body
    )
  end
  ...
  describe "accessing participatory texts in draft mode" do
    let!(:fiction) { create :fiction, :draft, component: current_component, participatory_text_level: "section" }

    it "renders only draft fictions" do
      visit_participatory_texts
  ...
  describe "discarding participatory texts in draft mode" do
    let!(:fictions) { create_list(:fiction, 5, :draft, component: current_component, participatory_text_level: "article") }

    it "removes all fictions in draft mode" do
      visit_participatory_texts
  ...
  describe "updating participatory texts in draft mode" do
    let!(:fiction) { create :fiction, :draft, component: current_component, participatory_text_level: "article" }
    let!(:new_body) { Faker::Lorem.unique.sentences(3).join("\n") }

    it "persists changes and all fictions remain as drafts" do
      visit_participatory_texts
      ...
      validate_occurrences(sections: 0, subsections: 0, articles: 1)
      fiction.reload
      expect(fiction.body.delete("\r")).to eq(new_body)
    end
  end

...
module Decidim
  module Fictions
    module Admin
      describe FictionAnswerForm do
        subject { form }

        let(:organization) { fictions_component.participatory_space.organization }
        let(:state) { "accepted" }
        let(:answer) { Decidim::Faker::Localized.sentence(3) }
        let(:fictions_component) { create :fiction_component }
        let(:cost) { nil }
        let(:cost_report) { nil }
        ...
        let(:form) do
          described_class.from_params(params).with_context(
            current_component: fictions_component,
        ...
        context "when rejecting a fiction" do
          let(:state) { "rejected" }
        ...
        context "when accepting the fiction" do
          let(:state) { "accepted" }

          context "and costs are enabled" do
            before do
              fictions_component.update!(
                step_settings: {
                  fictions_component.participatory_space.active_step.id => {

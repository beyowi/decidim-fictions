...
module Decidim
  module Fictions
    ...
    class CollaborativeDraftsController < Decidim::Fictions::ApplicationController
      ...
      def compare
        @step = :step_2
        @similar_collaborative_drafts ||= Decidim::Fictions::SimilarCollaborativeDrafts
        ...
        if @similar_collaborative_drafts.blank?
          flash[:notice] = I18n.t("fictions.collaborative_drafts.compare.no_similars_found", scope: "decidim")
      ...
      def create
        ...
        CreateCollaborativeDraft.call(@form, current_user) do
          on(:ok) do |collaborative_draft|
            flash[:notice] = I18n.t("fictions.collaborative_drafts.create.success", scope: "decidim")

            redirect_to Decidim::ResourceLocatorPresenter.new(collaborative_draft).path
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("fictions.collaborative_drafts.create.error", scope: "decidim")
            render :complete
          end
        end
      end
      ...
      def update
        ...
        UpdateCollaborativeDraft.call(@form, current_user, @collaborative_draft) do
          on(:ok) do |collaborative_draft|
            flash[:notice] = I18n.t("fictions.collaborative_drafts.update.success", scope: "decidim")
            redirect_to Decidim::ResourceLocatorPresenter.new(collaborative_draft).path
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("fictions.collaborative_drafts.update.error", scope: "decidim")
            render :edit
          end
        end
      end
      ...
      def withdraw
        WithdrawCollaborativeDraft.call(@collaborative_draft, current_user) do
          on(:ok) do
            flash[:notice] = t("withdraw.success", scope: "decidim.fictions.collaborative_drafts.collaborative_draft")
          end

          on(:invalid) do
            flash.now[:alert] = t("withdraw.error", scope: "decidim.fictions.collaborative_drafts.collaborative_draft")
          end
        end
        redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
      end
      ...
      def publish
        PublishCollaborativeDraft.call(@collaborative_draft, current_user) do
          on(:ok) do |fiction|
            flash[:notice] = I18n.t("publish.success", scope: "decidim.fictions.collaborative_drafts.collaborative_draft")
            redirect_to Decidim::ResourceLocatorPresenter.new(fiction).path
          end

          on(:invalid) do
            flash.now[:alert] = t("publish.error", scope: "decidim.fictions.collaborative_drafts.collaborative_draft")
            redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
          end
        end
      end
      ...
      def form_presenter
        @form_presenter ||= present(@form, presenter_class: Decidim::Fictions::CollaborativeDraftPresenter)
      end

...
module Decidim
  module Fictions
    module Admin
      # A form object to be used when admin users want to import a collection of fictions
      # from a participatory text.
      class ImportParticipatoryTextForm < Decidim::Form
        include TranslatableAttributes

        ACCEPTED_MIME_TYPES = Decidim::Fictions::DocToMarkdown::ACCEPTED_MIME_TYPES
        ...
        # Assume it's a NEW participatory_text if there are no fictions
        # Validate document presence while CREATING fictions from document
        # Allow skipping document validation while UPDATING title/description
        def new_participatory_text?
          Decidim::Fictions::Fiction.where(component: current_component).blank?
        end
        ...
        def valid_mime_types
          ACCEPTED_MIME_TYPES.values + [Decidim::Fictions::DocToMarkdown::TEXT_PLAIN_MIME_TYPE]
        end
        ...
        def i18n_valid_mime_types_text
          ACCEPTED_MIME_TYPES.keys.map do |mime_type|
            I18n.t(mime_type, scope: "decidim.fictions.admin.participatory_texts.new_import.accepted_mime_types")
          end.join(", ")
        end

...
module Decidim
  module Fictions
    ...
    # fictions in a participatory process.
    class FictionSearch < ResourceSearch
      # Public: Initializes the service.
      # component     - A Decidim::Component to get the fictions from.
      # page        - The page number to paginate the results.
      # per_page    - The number of fictions to return per page.
      def initialize(options = {})
        @component = options[:component]
        @current_user = options[:current_user]

        base = options[:state]&.member?("withdrawn") ? Fiction.withdrawn : Fiction.except_withdrawn
        super(base, options)
      end
      ...
      def search_search_text
        query
          .where("decidim_fictions_fictions.title ILIKE ?", "%#{search_text}%")
          .or(query.where("decidim_fictions_fictions.body ILIKE ?", "%#{search_text}%"))
      end
      ...
      def search_activity
        ...
        when "voted"
          query
            .includes(:votes)
            .where(decidim_fictions_fiction_votes: { decidim_author_id: @current_user })
        when "my_fictions"
          query
      ...
      # Filters Fictions by the name of the classes they are linked to. By default,
      # returns all Fictions. When a `related_to` param is given, then it camelcases item
      # to find the real class name and checks the links for the Fictions.
      #
      ...
      #
      # Returns only those fictions that are linked to the given class name.
      ...
      def results
        Fiction.where(id: super.pluck(:id))
      end
    end
  end
end

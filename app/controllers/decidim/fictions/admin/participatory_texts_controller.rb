  ...
  module Fictions
    module Admin
      ...
      class ParticipatoryTextsController < Admin::ApplicationController
        ...
        helper Decidim::Fictions::ParticipatoryTextsHelper

        def index
          @drafts = Fiction.where(component: current_component).drafts.order(:position)
        ...
        def import
          ...
          Admin::ImportParticipatoryText.call(@import) do
            on(:ok) do
              flash[:notice] = I18n.t("participatory_texts.import.success", scope: "decidim.fictions.admin")
              redirect_to EngineRouter.admin_proxy(current_component).participatory_texts_path
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("participatory_texts.import.invalid", scope: "decidim.fictions.admin")
              render action: "new_import"
            end

            on(:invalid_file) do
              flash.now[:alert] = I18n.t("participatory_texts.import.invalid_file", scope: "decidim.fictions.admin")
              render action: "new_import"
            end
          end
        end

        # When `save_draft` param exists, fictions are only saved.
        # When no `save_draft` param is set, fictions are saved and published.
        def update
          enforce_permission_to :manage, :participatory_texts

          form_params = params.require(:preview_participatory_text).permit!
          @preview_form = form(Admin::PreviewParticipatoryTextForm).from_params(fictions: form_params[:fictions_attributes]&.values)

          if params.has_key?("save_draft")
            UpdateParticipatoryText.call(@preview_form) do
              on(:ok) do
                flash[:notice] = I18n.t("participatory_texts.update.success", scope: "decidim.fictions.admin")
                redirect_to EngineRouter.admin_proxy(current_component).participatory_texts_path
              end

              on(:invalid) do |failures|
                alert_msg = [I18n.t("participatory_texts.publish.invalid", scope: "decidim.fictions.admin")]
            ...
            PublishParticipatoryText.call(@preview_form) do
              on(:ok) do
                flash[:notice] = I18n.t("participatory_texts.publish.success", scope: "decidim.fictions.admin")
        ...
        # Removes all the unpublished fictions (drafts).
        def discard
          enforce_permission_to :manage, :participatory_texts

          DiscardParticipatoryText.call(current_component) do
            on(:ok) do
              flash[:notice] = I18n.t("participatory_texts.discard.success", scope: "decidim.fictions.admin")
              redirect_to EngineRouter.admin_proxy(current_component).participatory_texts_path
            end
          end
        end

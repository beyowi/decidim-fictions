...
module Decidim
  module Fictions
    # Simple helpers to handle markup variations for fiction votes partials
    module FictionVotesHelper
      # Returns the css classes used for fiction votes count in both fictions list and show pages
      #
      # from_fictions_list - A boolean to indicate if the template is rendered from the fictions list page
      #
      # Returns a hash with the css classes for the count number and label
      def votes_count_classes(from_fictions_list)
        return { number: "card__support__number", label: "" } if from_fictions_list

        { number: "extra__suport-number", label: "extra__suport-text" }
      end

      # Returns the css classes used for fiction vote button in both fictions list and show pages
      #
      # from_fictions_list - A boolean to indicate if the template is rendered from the fictions list page
      #
      # Returns a string with the value of the css classes.
      def vote_button_classes(from_fictions_list)
        return "card__button" if from_fictions_list
      ...
      # Public: Checks if threshold per fiction are set.
      #
      # Returns true if set, false otherwise.
      def threshold_per_fiction_enabled?
        threshold_per_fiction.present?
      end

      # Public: Fetches the maximum amount of votes per fiction.
      #
      # Returns an Integer with the maximum amount of votes, nil otherwise.
      def threshold_per_fiction
        return nil unless component_settings.threshold_per_fiction.positive?

        component_settings.threshold_per_fiction
      end
      ...
      def remaining_votes_count_for(user)
        ...
        fictions = Fiction.where(component: current_component)

...
module Decidim
  module Fictions
    module Metrics
      class FictionsMetricManage < Decidim::MetricManage
        def metric_name
          "fictions"
        end
        ...
        def query
          ...
          @query = @query.where("decidim_fictions_fictions.published_at <= ?", end_time).except_withdrawn.not_hidden
        ...
        def quantity
          @quantity ||= query.where("decidim_fictions_fictions.published_at >= ?", start_time).count
        end
      end
    end
  end

...
shared_examples "when managing fictions category as an admin" do
  ...
  let!(:fiction_first) { reportables.first }
  let!(:fiction_last) { reportables.last }
    ...
    context "when selecting fictions" do
      ...
      context "when submiting form" do
        before do
          click_button "Actions"
          click_button "Change category"
          within "#js-form-recategorize-fictions" do

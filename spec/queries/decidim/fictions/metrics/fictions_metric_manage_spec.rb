...
describe Decidim::Fictions::Metrics::FictionsMetricManage do
  let(:organization) { create(:organization) }
  let(:participatory_space) { create(:participatory_process, :with_steps, organization: organization) }
  let(:component) { create(:fiction_component, :published, participatory_space: participatory_space) }
  let(:day) { Time.zone.yesterday }
  let!(:fictions) { create_list(:fiction, 3, published_at: day, component: component) }
  ...
  context "when executing" do
    ...
    it "updates metric records" do
      create(:metric, metric_type: "fictions", day: day, cumulative: 1, quantity: 1, organization: organization, category: nil, participatory_space: participatory_space)
      registry = generate_metric_registry
    ...
    context "when calculating the metrics" do
      let(:moderation) { create(:moderation, reportable: fictions[0], report_count: 1, participatory_space: participatory_space) }
      let!(:report) { create(:report, moderation: moderation) }

      it "filters the data correctly" do
        fictions[0].moderation.update!(hidden_at: Time.current)
        fictions[1].update!(published_at: nil)
        fictions[2].update!(state: "withdrawn")

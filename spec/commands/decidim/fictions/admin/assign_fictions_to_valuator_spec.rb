...
module Decidim
  module Fictions
    module Admin
      describe AssignFictionsToValuator do
        describe "call" do
          ...
          describe "when the form is valid" do
            ...
            it "creates the valuation assignment between the user and the fiction" do
              expect do
                command.call
              end.to change { ValuationAssignment.where(fiction: fiction, valuator_role: valuator_role).count }.by(1)
            end

            it "traces the action", versioning: true do
              expect(Decidim.traceability)
                .to receive(:create!)
                .with(Decidim::Fictions::ValuationAssignment, form.current_user, fiction: fiction, valuator_role: valuator_role)
                .and_call_original
            ...
            context "when it raises an error while creating assignments" do
              before do
                allow(Decidim::Fictions::ValuationAssignment).to receive(:create!).and_raise(ActiveRecord::RecordInvalid)
              end

...
describe "Explore Collaborative Drafts", versioning: true, type: :system do
  ...
  let(:request_access_form) { Decidim::Fictions::RequestAccessToCollaborativeDraftForm.from_params(state: collaborative_draft.state, id: collaborative_draft.id) }
  let!(:other_user) { create :user, :confirmed, organization: organization }
  let(:request_access_from_other_user) { Decidim::Fictions::RequestAccessToCollaborativeDraft.new(request_access_form, other_user) }
  ...
  context "with collaborative drafts enabled" do
    ...
    describe "Renders collaborative drafts index" do
      ...
      it "renders links to each collaborative draft details" do
        collaborative_drafts_count = Decidim::Fictions::CollaborativeDraft.open.where(component: component).count
    ...
    describe "renders collaborative draft details" do
      ...
      context "when publishing as a fiction" do
        before do
        ...
        context "when the published" do
          ...
          after do
            click_button "Publish as a Fiction"
          end
          ...
          it "shows the a modal" do
            within "[id$='publish-irreversible-action-modal'" do
              ...
              expect(page).to have_css("button", text: "Publish as a Fiction")
            end
          end
        end
      end
  ...
  context "with collaborative drafts disabled" do
    let(:component) { create(:fiction_component, manifest: manifest, participatory_space: participatory_process) }

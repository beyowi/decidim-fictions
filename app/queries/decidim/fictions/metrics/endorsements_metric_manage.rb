    ...
    module Metrics
      class EndorsementsMetricManage < Decidim::MetricManage
        ...
        def query
          ...
          fictions = Decidim::Fictions::Fiction.where(component: components).except_withdrawn
          join_components = "INNER JOIN decidim_components ON decidim_components.manifest_name = 'fictions' AND fictions.decidim_component_id = decidim_components.id"
          join_categories = <<~EOJOINCATS
            LEFT OUTER JOIN decidim_categorizations
            ON (fictions.id = decidim_categorizations.categorizable_id
            AND decidim_categorizations.categorizable_type = 'Decidim::Fictions::Fiction')
          EOJOINCATS
          @query = Decidim::Endorsement.joins("INNER JOIN decidim_fictions_fictions fictions ON resource_id = fictions.id")
                                       .joins(join_components)
                                       .joins(join_categories)
                                       .where(resource_id: fictions.pluck(:id))
                                       .where(resource_type: Decidim::Fictions::Fiction.name)

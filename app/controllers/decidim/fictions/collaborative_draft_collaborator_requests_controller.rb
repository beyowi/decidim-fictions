...
module Decidim
  module Fictions
    ...
    class CollaborativeDraftCollaboratorRequestsController < Decidim::Fictions::CollaborativeDraftsController
      ...
      def request_access
        ...
        RequestAccessToCollaborativeDraft.call(@request_access_form, current_user) do
          on(:ok) do |_collaborative_draft|
            flash[:notice] = t("access_requested.success", scope: "decidim.fictions.collaborative_drafts.requests")
          end

          on(:invalid) do
            flash[:alert] = t("access_requested.error", scope: "decidim.fictions.collaborative_drafts.requests")
          end
        end
      ...
      def request_accept
        ...
        AcceptAccessToCollaborativeDraft.call(@accept_request_form, current_user) do
          on(:ok) do |requester_user|
            flash[:notice] = t("accepted_request.success", scope: "decidim.fictions.collaborative_drafts.requests", user: requester_user.nickname)
          end

          on(:invalid) do
            flash[:alert] = t("accepted_request.error", scope: "decidim.fictions.collaborative_drafts.requests")
          end
        end
        redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
      end
      ...
      def request_reject
        ...
        RejectAccessToCollaborativeDraft.call(@reject_request_form, current_user) do
          on(:ok) do |requester_user|
            flash[:notice] = t("rejected_request.success", scope: "decidim.fictions.collaborative_drafts.requests", user: requester_user.nickname)
          end

          on(:invalid) do
            flash.now[:alert] = t("rejected_request.error", scope: "decidim.fictions.collaborative_drafts.requests")
          end
        end
        redirect_to Decidim::ResourceLocatorPresenter.new(@collaborative_draft).path
      end

    ...
    module Admin
      describe FictionsMergeForm do
        ...
        context "when not enough fictions" do
          let(:fictions) { create_list(:fiction, 1, component: component) }
        ...
        context "when merging to the same component" do
          ...
          context "when a fiction has an endorsement" do
            before do
              create(:endorsement, resource: fictions.sample, author: create(:user, organization: component.participatory_space.organization))
            end

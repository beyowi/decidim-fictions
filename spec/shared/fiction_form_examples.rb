...
shared_examples "a fiction form" do |options|
  subject { form }
  ...
  context "when geocoding is enabled" do
    let(:component) { create(:fiction_component, :with_geocoding_enabled, participatory_space: participatory_space) }
  ...
  it "properly maps category id from model" do
    fiction = create(:fiction, component: component, category: category)

    expect(described_class.from_model(fiction).category_id).to eq(category_id)
  end

  if options && options[:user_group_check]
    it "properly maps user group id from model" do
      fiction = create(:fiction, component: component, users: [author], user_groups: [user_group])

      expect(described_class.from_model(fiction).user_group_id).to eq(user_group_id)
    end
  end
...
shared_examples "a fiction form with meeting as author" do |options|
  subject { form }

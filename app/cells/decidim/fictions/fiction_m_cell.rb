...
module Decidim
  module Fictions
    # This cell renders a fiction with its M-size card.
    class FictionMCell < Decidim::CardMCell
      ...
      def badge_classes
        ...
        state_classes.concat(["label", "fiction-status"]).join(" ")
      end
      ...
      def progress_bar_subtitle_text
        if progress_bar_progress >= progress_bar_total
          t("decidim.fictions.fictions.votes_count.most_popular_fiction")
        else
          t("decidim.fictions.fictions.votes_count.need_more_votes")
        end
      end

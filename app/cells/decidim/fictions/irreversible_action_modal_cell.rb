  ...
  module Fictions
    ...
    class IrreversibleActionModalCell < Decidim::ViewModel
      ...
      def button_reveal_modal
        data = { open: modal_id }
        label = t(action, scope: "decidim.fictions.collaborative_drafts.show")
      ...
      def modal_title
        t("title", scope: "decidim.fictions.collaborative_drafts.collaborative_draft.#{action}.irreversible_action_modal")
      end

      def modal_body
        t("body", scope: "decidim.fictions.collaborative_drafts.collaborative_draft.#{action}.irreversible_action_modal")
      end

      def button_continue
        label = t("ok", scope: "decidim.fictions.collaborative_drafts.collaborative_draft.#{action}.irreversible_action_modal")
      ...
      def close_label
        t("cancel", scope: "decidim.fictions.collaborative_drafts.collaborative_draft.#{action}.irreversible_action_modal")
      end
      ...
      def resource_path(action)
        @resource_path ||= decidim_fictions.send("#{action}_collaborative_draft_path",
                                                  id: model.id)
      end

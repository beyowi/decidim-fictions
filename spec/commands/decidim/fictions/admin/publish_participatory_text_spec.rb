    ...
    module Admin
      describe PublishParticipatoryText do
        describe "call" do
          ...
          let(:fictions) do
            fictions = create_list(:fiction, 3, :draft, component: current_component)
          ...
          it "creates a version for each fiction", versioning: true do
            expect { command.call }.to broadcast(:ok)

            fictions.each do |fiction|
              expect(fiction.reload.versions.count).to eq(1)
            end
          end

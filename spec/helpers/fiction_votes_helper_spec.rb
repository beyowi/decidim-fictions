...
module Decidim
  module Fictions
    describe FictionVotesHelper do
      ...
      before do
        ...
        allow(helper).to receive(:current_component).and_return(fiction_component)
      ...
      describe "#vote_button_classes" do
        it "returns small buttons classes from fictions list" do
          expect(helper.vote_button_classes(true)).to eq("card__button")
        end

        it "returns expanded buttons classes if it's not from fictions list'" do
          expect(helper.vote_button_classes(false)).to eq("expanded")
        end
      end

      describe "#votes_count_classes" do
        it "returns small count classes from fictions list" do
          expect(helper.votes_count_classes(true)).to eq(number: "card__support__number", label: "")
        end

        it "returns expanded count classes if it's not from fictions list'" do
      ...
      describe "#remaining_votes_count_for" do
        it "returns the remaining votes for a user based on the component votes limit" do
          fiction = create(:fiction, component: fiction_component)
          create(:fiction_vote, author: user, fiction: fiction)
